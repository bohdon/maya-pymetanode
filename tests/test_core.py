...
class TestMetaData(unittest.TestCase):
    ...
    def test_set_and_get_data(self):
        ...
        meta.set_meta_data(self.node, class_name, set_data)
        self.assertEqual(meta.get_meta_data(self.node, class_name), set_data)

    def test_multi_class_data(self):
        cls1 = "myMetaClass1"
        cls2 = "myMetaClass2"
        meta.set_meta_data(self.node, cls1, None)
        meta.set_meta_data(self.node, cls2, None)
        self.assertEqual(meta.get_meta_data(self.node), {cls1: None, cls2: None})

    def test_remove_data(self):
        meta.set_meta_data(self.node, "myMetaClass", None)
        self.assertTrue(meta.is_meta_node(self.node))
        result = meta.remove_meta_data(self.node)
        self.assertTrue(result)
        self.assertFalse(meta.is_meta_node(self.node))

    def test_remove_locked_data(self):
        meta.set_meta_data(self.node, "myMetaClass", "myTestData")
        self.node.attr(meta.core.METADATA_ATTR).setLocked(True)
        result = meta.remove_meta_data(self.node)
        self.assertFalse(result)
        data = meta.get_meta_data(self.node)
        self.assertEqual(data, {"myMetaClass": "myTestData"})

    def test_remove_class_data(self):
        meta.set_meta_data(self.node, "myMetaClass", None)
        meta.set_meta_data(self.node, "mySecondMetaClass", None)
        result = meta.remove_meta_data(self.node, "myMetaClass")
        self.assertTrue(result)
        self.assertTrue(meta.is_meta_node(self.node))
        self.assertFalse(meta.has_meta_class(self.node, "myMetaClass"))
        self.assertTrue(meta.has_meta_class(self.node, "mySecondMetaClass"))
...
class TestNodeFinding(unittest.TestCase):
    def setUp(self):
        ...
        meta.set_meta_data(self.nodeA, "ClassA", "A")
        self.nodeB = pm.group(empty=True)
        meta.set_meta_data(self.nodeB, "ClassB", "B")
        meta.set_meta_data(self.nodeB, "ClassD", "D")
        self.nodeC = pm.group(empty=True)
        meta.set_meta_data(self.nodeC, "ClassC", "C")
        meta.set_meta_data(self.nodeC, "ClassD", "D")
